        ...
        <div class="notification-dropdown" (click)="$event.stopPropagation()">
          ...
          @if (notifications().length > 0) {
            <div class="dropdown-footer">
              <a routerLink="/collections/notifications" (click)="closeDropdown()">Xem tất cả</a>
            </div>
          }
        </div>
      }
...
export class NotificationBellComponent implements OnInit, OnDestroy {
  ...
  loadNotifications(): void {
    ...
    this.api.getAdminNotifications({ limit: 20 })
      ...
      .subscribe({
        ...
        error: (error) => {
          console.error('[Admin Notification] Lỗi', error);
          // Only show error if it's not a network error or auth error
          if (error.status !== 0 && error.status !== 401) {
            this.notifier.showError('Không thể tải thông báo');
          }
          this.loading.set(false);
        }
      });
  ...
  startPolling(): void {
    ...
    this.pollingInterval = setInterval(() => {
      if (!this.showDropdown()) {
        ...
        this.api.getAdminNotifications({ limit: 1, unreadOnly: true })
          .pipe(takeUntilDestroyed(this.destroyRef))
          .subscribe({
            next: (response) => {
            ...
            error: (error) => {
              // Silent fail for polling, but log for debugging
              if (error.status !== 0 && error.status !== 401) {
                console.debug('[Admin Notification] Polling error:', error?.message || error);
              }
            }
          });
      } else {
    ...
    this.api.markAllAsRead('admin')
      ...
      .subscribe({
        next: () => {
          ...
          this.notifier.showSuccess('Đã đánh dấu tất cả đã đọc');
        },
        error: (error) => {
          console.error('Error marking all as read:', error);
          this.markingAll.set(false);
          this.notifier.showError('Không thể đánh dấu tất cả đã đọc');
        }
      });
  }
